...
public class NumberGuessingGame {
    ...
    public static void main(String[] args) {
        ...
        while (!guessedCorrectly) {
            ...
            if (low > high) {
                System.out.println("Hmm, the feedback seems inconsistent. Please start over.");
                break;
            }
        }
        scanner.close();
    ...
    // Generates a random guess within the range [low, high]
    public static int generateGuess(int low, int high) {
        Random random = new Random();
        return random.nextInt(high - low + 1) + low; // Generate a random number between low and high
    }
}
