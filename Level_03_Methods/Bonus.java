...
public class Bonus {
    ...
    public static double[][] generateSalaryAndYears(int numEmployees) {
        ...
        for (int i = 0; i < numEmployees; i++) {
            salaryAndYears[i][0] = 10000 + random.nextInt(90000); // 5-digit salary
            salaryAndYears[i][1] = 1 + random.nextInt(20); // Years of service between 1 and 20
        }
    ...
    public static double[][] calculateNewSalaryAndBonus(double[][] salaryAndYears) {
        ...
        for (int i = 0; i < salaryAndYears.length; i++) {
            ...
            newSalaryAndBonus[i][2] = bonus;         // Bonus amount
        }
